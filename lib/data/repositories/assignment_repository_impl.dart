...
class AssignmentRepositoryImpl implements IAssignmentRepository {
  ...
  Future<List<AssignmentEntity>> getAssignmentsByCourse(String courseId) async {
    try {
      final assignmentModels = await remoteDataSource.getAssignmentsByCourse(courseId);
  ...
  @override
  Future<List<AssignmentEntity>> getAssignmentsByGroup(String courseId, String groupId) async {
    try {
      final assignmentModels = await remoteDataSource.getAssignmentsByGroup(courseId, groupId);
      return assignmentModels.map((model) => model.toEntity()).toList();
    } catch (e) {
      rethrow;
    }
  ...
  Future<AssignmentEntity?> getAssignmentById(String assignmentId) async {
    try {
      final assignmentModel = await remoteDataSource.getAssignmentById(assignmentId);
  ...
  Future<List<AssignmentEntity>> getOpenAssignments(String courseId) async {
    try {
      final assignmentModels = await remoteDataSource.getOpenAssignments(courseId);
      return assignmentModels.map((model) => model.toEntity()).toList();
    } catch (e) {
      rethrow;
    }
  ...
  @override
  Future<List<AssignmentEntity>> getUpcomingAssignments(String courseId, int daysAhead) async {
    try {
      final assignmentModels = await remoteDataSource.getUpcomingAssignments(courseId, daysAhead);
      return assignmentModels.map((model) => model.toEntity()).toList();
    } catch (e) {
      rethrow;
    }
  ...
  Future<AssignmentEntity> createAssignment(AssignmentEntity assignment) async {
    try {
      ...
      final createdModel = await remoteDataSource.createAssignment(assignmentModel);
  ...
  Future<AssignmentEntity> updateAssignment(AssignmentEntity assignment) async {
    try {
      final assignmentModel = AssignmentModel.fromEntity(assignment);
      final updatedModel = await remoteDataSource.updateAssignment(assignmentModel);
