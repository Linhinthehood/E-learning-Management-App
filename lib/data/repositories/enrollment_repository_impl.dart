...
class EnrollmentRepositoryImpl implements IEnrollmentRepository {
  ...
  Future<List<EnrollmentEntity>> getEnrollmentsByCourse(String courseId) async {
    try {
      final enrollmentModels = await remoteDataSource.getEnrollmentsByCourse(
        courseId,
      );
  ...
  Future<List<EnrollmentEntity>> getEnrollmentsByStudent(
    ...
    try {
      final enrollmentModels = await remoteDataSource.getEnrollmentsByStudent(
        studentId,
        semesterId,
      );
      return enrollmentModels.map((model) => model.toEntity()).toList();
  ...
  Future<EnrollmentEntity> createEnrollment(EnrollmentEntity enrollment) async {
    try {
      ...
      final createdModel = await remoteDataSource.createEnrollment(
        enrollmentModel,
      );
  ...
  Future<bool> isStudentEnrolledInCourse(
    ...
    try {
      return await remoteDataSource.isStudentEnrolledInCourse(
        studentId,
        courseId,
      );
    } catch (e) {
